"""Этап 4.5: структурный матчер — многоканальная NCC семантических карт.

Сравнивает карту рёбер самолёта (Canny на BEV-кадре) с растеризованными
векторными слоями Overture. Векторные данные инвариантны к сезону, поэтому
метод работает там, где appearance-матчер (XFeat) отказывает из-за снега.
"""
...
def _drone_edge_map(frame_bev: np.ndarray, lo: int = 50, hi: int = 150) -> np.ndarray:
    if frame_bev.ndim == 3:
...
def _per_class_channels(mask_v7: np.ndarray) -> dict[int, np.ndarray]:
    return {
        cid: ((mask_v7 == cid).astype(np.uint8) * 255)
        for cid in (CID_WATER, CID_FOREST_EDGE, CID_ROADS,
                    CID_FIELD_BOUNDARY, CID_BUILT_UP)
    }
...
class StructuralMatcher:
    ...
    def __init__(
        ...
        self.class_weights = dict(class_weights) if class_weights else _DEFAULT_CLASS_WEIGHTS
    ...
    def match(
        ...
        e_drone = _drone_edge_map(frame_bev)
        n_drone_edges = int((e_drone > 0).sum())
        if n_drone_edges < 200:
            return StructuralFix(reject_reason="too_few_drone_edges",
                                 n_drone_edges=n_drone_edges)

        half_extent_m = 0.5 * bev_ground_span_m + self.search_radius_m
        cos_lat = math.cos(math.radians(seed_lat))
        d_lat = half_extent_m / 111320.0
        d_lon = half_extent_m / (111320.0 * cos_lat)
        bbox = (seed_lon - d_lon, seed_lat - d_lat,
                seed_lon + d_lon, seed_lat + d_lat)
        ...
        if n_sat_features < 1000:
            return StructuralFix(reject_reason="too_few_sat_features",
        ...
        for cid, channel in sat_channels.items():
            ...
            if w <= 0:
                continue
            ncc = _ncc_sliding(e_drone, channel)
            if ncc.size == 0:
                continue
        ...
        if score_sum is None or weight_sum == 0:
            return StructuralFix(reject_reason="no_valid_channels",
                                 n_drone_edges=n_drone_edges,
                                 n_sat_features=n_sat_features)
        ...
        x0 = max(0, peak_x - nbr); x1 = min(score_map.shape[1], peak_x + nbr + 1)
        y0 = max(0, peak_y - nbr); y1 = min(score_map.shape[0], peak_y + nbr + 1)
