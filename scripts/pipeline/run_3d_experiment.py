...
def _draw_tracking_state(img: np.ndarray, state: str, confidence: float) -> np.ndarray:
    color = (40, 220, 40)
    if state == "WEAK":
        color = (0, 200, 255)
    ...
    cv2.putText(img, label, (20, 34), cv2.FONT_HERSHEY_SIMPLEX, 0.78, (0, 0, 0), 4, cv2.LINE_AA)
    cv2.putText(img, label, (20, 34), cv2.FONT_HERSHEY_SIMPLEX, 0.78, color, 2, cv2.LINE_AA)
    return img
...
def _draw_geometry_debug(
    ...
    cv2.putText(out, text, (20, 64), cv2.FONT_HERSHEY_SIMPLEX, 0.55, (0, 0, 0), 3, cv2.LINE_AA)
    cv2.putText(out, text, (20, 64), cv2.FONT_HERSHEY_SIMPLEX, 0.55, (255, 230, 80), 1, cv2.LINE_AA)
    return out
...
def generate_final_video(video_path: str, output_path: str, duration_sec: int = 20, fps_out: int = 15):
    ...
    def render_side_by_side(drone_img, map_img, pts_drone, pts_map, stable_mask=None, max_points=100):
        """Рисует склеенное изображение с линиями, унифицировано для всех кадров"""
        if len(drone_img.shape) == 2:
            drone_img = cv2.cvtColor(drone_img, cv2.COLOR_GRAY2BGR)

        drone_img = _overlay_ground_mask(drone_img, stable_mask)
    ...
    def filter_matches_by_ground(stable_mask, pts_drone, pts_map):
        """Оставляет только точки в стабильных наземных регионах."""
        if len(pts_drone) == 0:
        ...
        valid_indices = []
        for point_idx, (x, y) in enumerate(pts_drone):
            x_int = int(round(x))
            y_int = int(round(y))
            if 0 <= x_int < w and 0 <= y_int < h and stable_mask[y_int, x_int] > 0:
    ...
    for i in tqdm(range(total_frames)):
        ...
        stable_mask = geo_segmentor.stable_ground_mask(frame)
        ...
        if needs_keyframe:
            ...
            # --- СЕГМЕНТАЦИЯ: оставляем только точки из стабильной наземной области ---
            mkpts_drone_seg, mkpts_map_seg = filter_matches_by_ground(stable_mask, raw_mkpts_drone, raw_mkpts_map)
            if len(mkpts_drone_seg) >= min_ground_points:
                mkpts_drone, mkpts_map = mkpts_drone_seg, mkpts_map_seg
                seg_filter_applied = True
            else:
                mkpts_drone, mkpts_map = raw_mkpts_drone, raw_mkpts_map
            ...
            base_img = render_side_by_side(frame, map_cv2, viz_mkpts_drone, viz_mkpts_map, stable_mask=stable_mask)
            ...
            if p1 is None or st is None:
                ...
                base_img = render_side_by_side(frame, map_cv2, mkpts_drone, mkpts_map, stable_mask=stable_mask)
                prev_gray_drone = drone_gray.copy()
                ...
                final_img = _draw_tracking_state(final_img, tracking_state, last_confidence)
            ...
            mkpts_drone_seg, mkpts_map_seg = filter_matches_by_ground(stable_mask, mkpts_drone, mkpts_map)
        ...
        base_img = _draw_geometry_debug(base_img, frame.shape[1], projection_polygon, match_stats)
        ...
        cv2.putText(base_img, match_line, (20, 88), cv2.FONT_HERSHEY_SIMPLEX, 0.55, (0, 0, 0), 3, cv2.LINE_AA)
        cv2.putText(base_img, match_line, (20, 88), cv2.FONT_HERSHEY_SIMPLEX, 0.55, (120, 255, 210), 1, cv2.LINE_AA)
